            ...
            html: `
                <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
                    <div style="background: #0a0a0a; padding: 32px; border-radius: 16px 16px 0 0; text-align: center;">
                        ...
                        <h1 style="color: #fff; margin-top: 16px; font-size: 1.5rem;">${escapeHtml(subject)}</h1>
                    </div>
                    <div style="background: #111; padding: 32px; border-radius: 0 0 16px 16px;">
                        <p style="color: #ccc; line-height: 1.7;">Hi ${escapeHtml(customerName || 'there')},</p>
                        <pre style="color: #ccc; white-space: pre-wrap; font-family: sans-serif; line-height: 1.7;">${escapeHtml(body)}</pre>
 ...
 * Language: Customer SMS is sent in their detected language (Spanish/English).
 */
...
View reasoning: ${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/dashboard`;
    ...
    // Customer message in their detected language
    const lang = booking.language || 'en';
    const customerMessage = lang === 'es'
        ? `Hola ${booking.customer_name}, soy Maya de Mr. Cleaner. Su cita de detallado está confirmada para el ${booking.booking_date} a las ${booking.booking_time}. ¡Estamos listos para restaurar su ${booking.vehicle_type}! Nos vemos pronto.`
        : `Hi ${booking.customer_name}, this is Maya from Mr. Cleaner. Your detailing appointment is confirmed for ${booking.booking_date} at ${booking.booking_time}. We're excited to restore your ${booking.vehicle_type}! See you soon.`;
...
Dashboard: ${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/dashboard`;
        ...
        const ownerPhone = business?.phone || process.env.BUSINESS_PHONE;
