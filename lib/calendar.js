...
export async function checkAvailability(date, requestedDuration = 120) {
    ...
    try {
        ...
        Sentry.captureException(error, { tags: { module: 'calendar', method: 'checkAvailability' } });
        return [];
    }
}
...
export async function createCalendarEvent(booking) {
    ...
    try {
        ...
        console.error(isTimeout ? 'Calendar API timed out' : 'Error creating event:', error.message);
        Sentry.captureException(error, { tags: { module: 'calendar', method: 'insertEvent', isTimeout: String(isTimeout) } });
...
export async function handleAuthCallback(code) {
    try {
        ...
        Sentry.captureException(error, { tags: { module: 'calendar', method: 'handleCallback' } });
