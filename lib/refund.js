 ...
 *
 * Steps:
 * 1. Fetch booking from Supabase (scoped to businessId)
 ...
 *
 * SECURITY: businessId is required — prevents cross-tenant refund attacks.
 * If the booking doesn't belong to this business, returns NOT_FOUND (no info leak).
 *
 * @param {string} bookingId - The booking UUID
 * @param {string} businessId - The business UUID (ownership check)
...
export async function processRefund(bookingId, businessId) {
    ...
    // 1. Fetch the booking — scoped to businessId to prevent cross-tenant refunds
