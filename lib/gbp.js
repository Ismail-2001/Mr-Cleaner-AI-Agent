...
export function verifyGoogleWebhook(body) {
    if (!GBP_PUBSUB_VERIFICATION_TOKEN) {
        ...
        if (process.env.NODE_ENV === 'production') {
            console.error('CRITICAL: GBP_PUBSUB_VERIFICATION_TOKEN not set — rejecting webhook (fail closed)');
            return false;
        }
        console.warn('GBP_PUBSUB_VERIFICATION_TOKEN not set — allowing webhook in development (INSECURE)');
        return true;
    }
